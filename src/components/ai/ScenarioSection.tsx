...
function SolutionCard({
  ...
  return (
    <div
      className={clsx(
        "group relative overflow-hidden transition-all duration-500 ease-in-out cursor-pointer border border-gray-200 rounded-none bg-white",
    ...
    >
      ...
      <div className="relative h-full flex flex-col p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3
             className="text-xl font-bold transition-all duration-300 text-gray-900 py-2"
             style={{
        ...
        <div className={clsx(
          ...
          <div className="space-y-3">
            <h4 className="text-base font-semibold text-gray-900 mb-3">核心功能</h4>
            {solution.features.map((feature, featureIndex) => (
              <div
                key={featureIndex}
                className={clsx(
...
function MobileSolutionCard({
  ...
  return (
    <div className="relative overflow-hidden border border-gray-200 h-[180px] xs:h-[220px] sm:h-[280px] group rounded-none bg-white">
      {/* 内容区域 */}
      <div className="relative h-full flex flex-col p-3 sm:p-4">
        ...
        {/* 描述文本 */}
        <p className="text-xs sm:text-sm leading-relaxed text-gray-600 mb-2 sm:mb-4 flex-1 line-clamp-3 sm:line-clamp-4">
          {solution.description}
        </p>

        {/* 核心功能列表 */}
        <div className="space-y-1 sm:space-y-2">
          <h4 className="text-xs sm:text-sm font-semibold text-gray-900">核心功能</h4>
          {solution.features.slice(0, 2).map((feature, featureIndex) => (
            <div
              key={featureIndex}
              className="flex items-center text-xs sm:text-sm text-gray-600"
            >
              <div className="mr-1.5 sm:mr-2 h-1 w-1 sm:h-1.5 sm:w-1.5 rounded-full bg-blue-500" />
              {feature}
            </div>
          ))}
  ...
  return (
    ...
    >
      <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: '1800px' }}>
        ...
        {/* PC端手风琴布局 - 隐藏在移动端 */}
        <div className="hidden lg:flex h-[400px] xl:h-[500px] gap-4 overflow-hidden mb-8">
