...
const faqCategories: FAQCategory[] = [
  {
    ...
    faqs: [
      {
        question: '什么是 CVM 实例？',
        answer: 'CVM（Cloud Virtual Machine）是优刻云提供的可扩展的虚拟计算服务。您可以使用 CVM 来构建和托管软件系统。CVM 提供多种实例类型，优化以适合不同的使用案例。实例类型包括不同的 CPU、内存、存储和网络容量组合，让您能够灵活地为应用程序选择适当的资源组合。'
      },
      {
        question: '如何开始使用 CVM？',
        answer: '开始使用 CVM 非常简单。首先注册优刻云账户，然后登录控制台，选择 CVM 产品，点击"立即购买"按钮。您可以选择适合的实例规格、操作系统、网络配置等。创建完成后，您就可以通过 SSH 或远程桌面连接到您的实例。我们还提供详细的快速入门指南帮助您完成首次配置。'
      },
      {
        question: '支持哪些操作系统实例？',
        answer: 'CVM 支持多种操作系统，包括：Linux 发行版（如 CentOS、Ubuntu、Debian、SUSE Linux）、Windows Server 各个版本、以及优刻云自研的 TencentOS Server。您可以根据应用需求选择最适合的操作系统。所有镜像都经过安全加固和性能优化，确保稳定可靠的运行环境。'
      }
    ]
  },
  {
    ...
    faqs: [
      ...
      {
        question: '如何查看和管理费用？',
        answer: '您可以通过优刻云控制台的"费用中心"查看详细的费用信息，包括：实时费用、历史账单、费用趋势分析等。系统支持设置费用预警，当费用达到设定阈值时会及时通知您。同时提供费用分析工具，帮助您了解各项服务的费用构成，优化成本结构。'
      },
      {
        question: '可以申请费用减免吗？',
        answer: '优刻云为新用户提供丰富的优惠活动，包括新用户代金券、首购优惠等。对于教育用户、开源项目、初创企业等，我们也有相应的扶持计划。具体的优惠政策和申请流程，请查看官网活动页面或联系客服咨询。我们致力于为不同类型的用户提供合适的优惠支持。'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    faqs: [
      ...
      {
        question: '如何保护实例免受攻击？',
        answer: '保护实例安全的建议：1）及时更新系统补丁；2）配置严格的安全组规则；3）使用强密码或密钥认证；4）安装防病毒软件；5）定期备份重要数据；6）监控异常登录和网络活动；7）使用优刻云安全产品如主机安全、DDoS防护等。建议采用纵深防御策略，多层保护。'
      },
      {
        question: '支持合规认证吗？',
        answer: '优刻云已通过多项国际和国内合规认证，包括：ISO 27001、SOC 1/2/3、PCI DSS、等保三级、可信云等。CVM 服务符合相关合规要求，可以满足金融、医疗、政务等行业的合规需求。我们持续投入合规建设，为用户提供可信赖的云计算服务。'
      }
    ]
  }
]
